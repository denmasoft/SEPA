...
abstract class BaseDomBuilder implements DomBuilderInterface
{
    ...
    public function visitGroupHeader(GroupHeader $groupHeader)
    {
        ...
        $groupHeaderTag->appendChild($this->createElement('NbOfTxs', $groupHeader->getNumberOfTransactions()/2));
        $groupHeaderTag->appendChild(
            $this->createElement('CtrlSum', $groupHeader->getControlSumCents()/2)
        );
        ...
        if ($groupHeader->getInitiatingPartyId() !== null) {
            ...
            $initiatingParty->appendChild($newId);
        }
