...
class CustomerDirectDebitTransferDomBuilder extends BaseDomBuilder
{
    ...
    public function visitPaymentInformation(PaymentInformation $paymentInformation)
    {
        ...
        $this->currentPayment->appendChild(
            $this->createElement('CtrlSum', $paymentInformation->getControlSumCents())
        );
       ...
       // $CtgyPurp->appendChild($this->createElement('Cd',$paymentInformation->getCtgyPurposeCode()));
        //$paymentTypeInformation->appendChild($CtgyPurp);
        ...
        $PstlAdr->appendChild($this->createElement('Ctry',$paymentInformation->getCountry()));
        foreach ($paymentInformation->getAddressLine() as $adrLine) {
            $PstlAdr->appendChild($this->createElement('AdrLine',$adrLine));
        }
        $creditor->appendChild($PstlAdr);
    ...
    public function visitTransferInformation(TransferInformationInterface $transactionInformation)
    {
        ...
        $PstlAdr = $this->createElement('PstlAdr');
        $PstlAdr->appendChild($this->createElement('Ctry',$transactionInformation->getCountry()));
        foreach ($transactionInformation->getAddressLine() as $adrLine) {
            $PstlAdr->appendChild($this->createElement('AdrLine',$adrLine));
        }
        $debtor->appendChild($this->createElement('Nm', $transactionInformation->getDebitorName()));
        $debtor->appendChild($PstlAdr);
        ...
        //$Purp = $this->createElement('Purp');
        //$Purp->appendChild($this->createElement('Cd',$transactionInformation->getPurposeCode()));
        //$directDebitTransactionInformation->appendChild($Purp);
